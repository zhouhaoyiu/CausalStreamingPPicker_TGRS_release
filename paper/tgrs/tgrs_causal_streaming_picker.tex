...
The external evidence covers labeled response and continuous false-trigger behavior. PNWAccelerometers gives 85.0\% usable response on high-SNR labeled accelerometer records and 30.4\% on an unfiltered low-SNR reference subset. CI/FDSN supplies event-free continuous-stream exposure. The release records CESMD, ESM, CWA/TSMIP, and AFAD as the next validation tier after P-reference provenance is documented. This evidence structure keeps earliest weak-P evidence, which controls warning time, separate from later strong-energy growth, which is closer to engineering ground-motion products.

The practical output is a station candidate; alerting is a downstream network decision. Two-packet confirmation reduces clean K-NET pre-P false triggers to 2.25 episodes per hour and preserves 99.3\% K-NET event detection. The spike, STEAD, and CI/FDSN audits define the station quality-control boundary around the candidate stream. Event-window association shows that the same triggers can form simple multi-station coincidences in catalog-grouped K-NET events. A complete alerting tier adds chronological station-day evaluation, blind network association, source location, magnitude estimation, telemetry latency, and hardware-in-the-loop tests. P/S joint outputs, probability calibration, and quantized ARM or MCU runtime tests are natural engineering extensions.
